...
/**
 * This integration test makes real request to the CloudProviderRanker APIs.
 * 
 * @author l.biava
 *
 */
public class CloudProviderRankerServiceIT extends WebAppConfigurationAwareIT {
  ...
  public static void mockCpr(MockRestServiceServer mockServer, URI baseUrl,
    ...
    mockServer.expect(requestTo(baseUrl + "/rank")).andExpect(method(HttpMethod.POST)).andRespond(
        withSuccess(JsonUtils.serialize(response), MediaType.APPLICATION_JSON));
  }

  @Test
  public void rankProviders() throws Exception {

    RankCloudProvidersMessage rcpm = JsonUtils.deserialize(
    ...
    mockCpr(mockServer, cprProperties.getUrl(), providers);
