...
@Component
@Slf4j
@EnableConfigurationProperties(RucioClientProperties.class)
public class RucioStompClient implements SmartLifecycle {
  private final RucioClientProperties stompClientProperties;
  ...
  public RucioStompClient(RucioClientProperties stompClientProperties, EventLoopGroup eventLoopGroup, Environment environment, RucioMessageHandlerFactory rucioMessageHandlerFactory) {
  ...
  public StompSession newStompSession(String host, int port) {
    ...
    StompSessionHandler connectionHandler = new StompSessionHandlerAdapter() {
      ...
      public void afterConnected(StompSession session, StompHeaders connectedHeaders) {
        ...
        RucioMessageHandler messageHandler = rucioMessageHandlerFactory.getObject(session);
  ...
  @Component
  public static class RucioMessageHandlerFactory {
    private final ObjectMapper objectMapper;
    private final RucioEventHandlerService rucioEventHandlerService;

    private RucioMessageHandlerFactory(ObjectMapper objectMapper, RucioEventHandlerService rucioEventHandlerService) {
      this.objectMapper = objectMapper;
      this.rucioEventHandlerService = rucioEventHandlerService;
    }

    public RucioMessageHandler getObject(StompSession session) {
      return new RucioMessageHandler(objectMapper, rucioEventHandlerService, session);
    }
  }

  public static class RucioMessageHandler implements StompFrameHandler {
    private final StompSession session;

    private final ObjectMapper objectMapper;
    private final RucioEventHandlerService rucioEventHandlerService;

    public RucioMessageHandler(ObjectMapper objectMapper, RucioEventHandlerService rucioEventHandlerService, StompSession session) {
      this.rucioEventHandlerService = rucioEventHandlerService;
